// GUESS THAT NUMBER
// Text/message to be used thorughout file
const enterNumText = `Please enter a number greater than zero`;
// For restarting the game
let restartGame = true;
// For storing the range of the number to be guessed
let rangeNum;
// For string the number to be guessed
let randomNum;
// For storing number of attampts left
let attempts;
...
while (restartGame){
    ...
    // Verifies the user's entry for a range number is a number greater than zero (NOTE: All numbers, positive and negative, have a default boolean value of true, except for zero which has a boolean value of false. Also, Nan has a default boolean value of false as well.)
    ...
    while (true){
        ...
        if (guess === `fluffy bunnies`){
            alert(`The number is ${randomNum}`);
            prompt(`Please enter a guess between 1 and ${attempts} attempts(s) left:`);
        }
        ...
        if (guess === randomNum){
            ...
            break;
            // Checks if user has any attempts left. If not, the game ends.
        } else if (attempts === 0){
            alert(`Sorry, but have run out of attempts :(
